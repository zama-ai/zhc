...
impl Builder {
    ...
    /// # use zhc_builder::*;
    /// let builder = Builder::new(CiphertextBlockSpec(2, 2));
    /// let c = builder.integer_ciphertext_input(8);
    /// let p = builder.integer_plaintext_input(8);
    /// let c_blocks = builder.integer_ciphertext_split(&c);
    /// let p_blocks = builder.integer_plaintext_split(&p);
    ...
    /// value whose estimate exceeds the budget is flagged with a warning mark. IntegerPlaintext
    /// values carry no noise and always show 0%.
    ///
