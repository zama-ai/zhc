use crate::{IntegerCiphertext, builder::Builder};
...
///
/// ```rust,no_run
/// # use zhc_builder::{IntegerCiphertextSpec, cast};
/// # let spec = IntegerCiphertextSpec::new(16, 2, 2);
/// let builder = cast(spec, 32);
/// let ir = builder.optimize_ir();
/// ```
pub fn cast(spec: IntegerCiphertextSpec, to_size: u16) -> Builder {
...
impl Builder {
    ...
    /// # let spec = IntegerCiphertextSpec::new(16, 2, 2);
    /// # let builder = Builder::new(spec.block_spec());
    /// # let ct = builder.integer_ciphertext_input(spec.int_size());
    /// let wide = builder.iop_cast(&ct, 32);
    /// ```
    // NOTE: unsigned only; sign extension unsuported in zhc today
    pub fn iop_cast(&self, src: &IntegerCiphertext, to_size: u16) -> IntegerCiphertext {
        ...
        self.integer_ciphertext_join(&src_blocks[..kept], Some(to_size))
    }
}
...
mod test {
    ...
    #[test]
    fn test_cast_widen() {
        let ir = cast(IntegerCiphertextSpec::new(8, 2, 2), 16);
        assert_display_is!(
            ir.optimize_ir()
                .format()
                .show_comments(false)
    ...
    #[test]
    fn test_cast_narrow() {
        let ir = cast(IntegerCiphertextSpec::new(16, 2, 2), 8);
        assert_display_is!(
            ir.optimize_ir()
                .format()
                .show_comments(false)
    ...
    fn correctness_cast() {
        fn semantic(to_size: u16) -> impl Fn(&[IopValue]) -> Option<Vec<IopValue>> {
            move |inp: &[IopValue]| {
                ...
                let to_spec = IntegerCiphertextSpec::new(to_size, 2, 2);
        ...
        for (from, to) in [
            ...
            cast(IntegerCiphertextSpec::new(from, 2, 2), to).test_random(100, semantic(to));
        }
    }
}
