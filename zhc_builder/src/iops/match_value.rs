...
/// Creates an IR that maps an encrypted integer through a clear table.
...
impl Builder {
    /// Maps an encrypted integer through a table of (input, output) pairs.
    /// Returns (output, matched). An unmatched input yields (0, 0).
    /// This is the contract of tfhe-rs `match_value_parallelized`, with a different construction.
    /// tfhe-rs one-hot selects over the table entries, so its cost scales with the entry count.
    /// This op muxes over the input space, so its cost scales with the input width
    /// regardless of table size (~200 PBS for the AES S-box).
    ///
    /// The op splits the input into a packed 4-bit low part and the remaining high bits.
    /// A PBS can only see the low part.
    /// For each possible high value `h`, a dedicated table-derived LUT maps the low part
    /// to the table output as if the high bits were `h`.
    /// This yields one candidate result per `h`.
    /// A mux tree then reads the encrypted high bits and selects the candidate for the
    /// actual high value.
    ///
    /// NOTE: candidates grow as 4^(blocks - 2), so the op caps the input at 8 bits.
    /// The AES S-box is the only consumer today.
    /// The op emits `Lut1Def::Table` LUTs.
    /// Lowering allocates gids for them, and the pipeline exposes their payload
    /// via `get_hpu_lut_payload`. The caller must upload the payload to the device.
    ///
    /// # Panics
    ///
    ...
    pub fn iop_match_value(
        ...
        // Only AES uses this IOp for now, so 8 bits is enough.
        // TODO: if a wider input is necessary one day, find a construction for more than 8 bits.
        assert!(
            in_size <= 8,
        ...
        // Dense view of the table, indexed by input value. The fill loop also validates the keys.
        // The view spans the packed input space, which odd `in_size`s do not fill.
        ...
        // Column j holds output digit j of every candidate. The last column holds the matched flag.
...
mod test {
    ...
    fn correctness_match_value() {
        ...
        // Full 8-bit permutation with an S-box shape (mul-swap-mul).
        // Every out digit depends on high input bits, so the test exercises the whole mux tree.
