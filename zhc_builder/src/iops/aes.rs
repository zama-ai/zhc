...
//! If a second bit-level circuit needs it, move it to its own module.
//!
...
//!
//! TOREVIEW: NU is set to a pessimistic value. A recompute at zhc compile time would improve
//! performance. It is safe to ignore here. Revert if a problem appears.
...
/// The noise budget gives this limit.
...
impl Builder {
    ...
    /// The function reduces an operand only if the sum exceeds the noise budget.
    ...
    /// data space.
    ///
    /// NOTE: the code ignores NU here. The pack has a
    /// noise of `(ty + 1) + ty`, which is 7 at a maximum.
    /// TOREVIEW: this adds 3 PBS to each S-box, which greatly decreases performance.
...
mod test {
    ...
    //
    // The count is 10,624, which is about 0.77 s on the device. The state
