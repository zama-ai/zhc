...
pub fn shiftrots(spec: IntegerCiphertextSpec, kind: ShiftRotKind) -> Builder {
    let builder = Builder::new(spec.block_spec());
    let src = builder.integer_ciphertext_input(spec.int_size());
    let amount = builder.integer_plaintext_input(spec.int_size());
...
impl Builder {
    ...
    /// # let amt = builder.integer_plaintext_input(spec.int_size());
...
mod test {
    ...
    fn exercise_scalar(kind: ShiftRotKind, int_size: u16, reps: usize) {
        ...
        let builder = Builder::new(spec.block_spec());
        let src = builder.integer_ciphertext_input(spec.int_size());
        let amount_pt = builder.integer_plaintext_input(spec.int_size());
