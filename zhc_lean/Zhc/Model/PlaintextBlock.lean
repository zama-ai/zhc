...
structure PlaintextBlock (sb : SpecBlock) where
  storage : Fin (2 ^ sb.completeSize)
  deriving DecidableEq, Repr

namespace PlaintextBlock variable {sb : SpecBlock}

def ofNat (v : Nat) : PlaintextBlock sb :=
  ⟨⟨v % 2 ^ sb.completeSize, Util.mod_two_pow_lt⟩⟩

def ofNat? (v : Nat) : Option (PlaintextBlock sb) :=
  if v < 2 ^ sb.completeSize then some (ofNat v) else none

def toCompleteNat (self : PlaintextBlock sb) : Nat :=
  self.storage.val

def toMessageNat (self : PlaintextBlock sb) : Nat :=
  self.toCompleteNat % 2 ^ sb.messageSize

abbrev hasCarryOnly (self : PlaintextBlock sb) : Prop :=
  self.toCompleteNat / 2 ^ sb.dataSize = 0

abbrev hasMessageOnly (self : PlaintextBlock sb) : Prop :=
  self.toCompleteNat / 2 ^ sb.messageSize = 0

@[simp] theorem toCompleteNat_ofNat (v : Nat) :
  (ofNat v : PlaintextBlock sb).toCompleteNat = v % 2 ^ sb.completeSize
:=
  rfl

theorem toCompleteNat_lt (a : PlaintextBlock sb) :
  a.toCompleteNat < 2 ^ sb.completeSize
:=
  a.storage.isLt

theorem ext_toCompleteNat {a b : PlaintextBlock sb} :
...
@[simp] theorem toMessageNat_eq (a : PlaintextBlock sb) :
  a.toMessageNat = a.toCompleteNat % 2 ^ sb.messageSize
:=
  rfl

@[simp] theorem hasMessageOnly_iff (a : PlaintextBlock sb) :
  a.hasMessageOnly ↔ a.toCompleteNat < 2 ^ sb.messageSize
:=
  Util.div_two_pow_eq_zero_iff

@[simp] theorem hasCarryOnly_iff (a : PlaintextBlock sb) :
  a.hasCarryOnly ↔ a.toCompleteNat < 2 ^ sb.dataSize
:=
  Util.div_two_pow_eq_zero_iff

def listToNat : List (PlaintextBlock sb) → Nat
  | [] => 0
  | b :: bs => b.toMessageNat + 2 ^ sb.messageSize * listToNat bs

@[simp] theorem listToNat_nil :
  listToNat ([] : List (PlaintextBlock sb)) = 0
:=
  rfl

@[simp] theorem listToNat_cons (b : PlaintextBlock sb) (bs : List (PlaintextBlock sb)) :
  listToNat (b :: bs) = b.toMessageNat + 2 ^ sb.messageSize * listToNat bs
:=
  rfl

theorem toMessageNat_lt (b : PlaintextBlock sb) :
  b.toMessageNat < 2 ^ sb.messageSize
:=
  Util.mod_two_pow_lt

theorem listToNat_lt (l : List (PlaintextBlock sb)) :
...
theorem toCompleteNat_getElem_of_clean (l : List (PlaintextBlock sb))
...
theorem listToNat_set_of_lt (l : List (PlaintextBlock sb)) (i : Nat) (b : PlaintextBlock sb) :
...
def digits (n v : Nat) : List (PlaintextBlock sb) :=
  match n with
  | 0 => []
