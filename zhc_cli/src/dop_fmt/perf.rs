...
mod tests {
    ...
    fn parses_report_names_case_insensitively() {
        assert_eq!(
            "latency".parse::<PerfReport>().unwrap(),
            PerfReport::Latency
        );
        assert_eq!(
        ...
        assert_eq!(
            "pbs-batch".parse::<PerfReport>().unwrap(),
            PerfReport::PbsBatch
        );
...
fn simulate(ir: &IR<DopLang>, config: &HpuConfig) -> (Cycle, Statistics) {
    let mut simulator =
        Simulator::from_simulatable(config.freq, Hpu::new(config, HpuId(0)), TracingLevel::None);
    let dops = ir
        .walk_ops_linear()
        .map(|op| DOp {
