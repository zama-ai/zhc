...
impl Addrs {
    fn visit_reg(&mut self, a: &CtReg) {
        ...
        self.max_reg = Some(self.max_reg.map_or(top as usize, |m| m.max(top as usize)));
    }

    fn visit_mem(&mut self, a: &CtMem) {
        match a {
            CtMem::Heap(inner) => {
                self.heap.insert(inner.addr as usize);
            }
            CtMem::Io(inner) => {
                self.io.insert(inner.addr as usize);
            }
            CtMem::Src(inner) => {
                self.ct_src
                    .insert((inner.id as usize, inner.block as usize));
            }
...
fn recompose(blocks: &[EmulatedCiphertextBlock], message_size: u8) -> usize {
    blocks.iter().enumerate().fold(0usize, |acc, (i, b)| {
...
pub fn simulate(
    ...
    for id in ts_ids {
        ...
        let values = match inputs.get(id) {
            ...
            None => (0..n)
                .map(|_| block_spec.random().raw_message_bits())
                .collect(),
        };
