...
///
/// This is the plaintext counterpart to
/// [`IntegerCiphertextSpec`](super::super::IntegerCiphertextSpec). It defines a large integer
/// decomposed into multiple plaintext blocks, where each block holds a portion of the integer
/// according to a shared [`PlaintextBlockSpec`].
///
...
impl IntegerPlaintextSpec {
    ...
    pub fn int_mask(&self) -> EmulatedIntegerPlaintextStorage {
        // NB: shift down a pattern of ones instead of `(1 << width) - 1`, whose shift overflows on
        // a full width (128 bit) integer.
        let width = (self.block_count() * self.block.message_size()).sas::<u32>();
        if width == 0 {
            return 0;
        }
        EmulatedIntegerPlaintextStorage::MAX >> (EmulatedIntegerPlaintextStorage::BITS - width)
    }
    ...
    pub fn overflows_int(&self, storage: EmulatedIntegerPlaintextStorage) -> bool {
        ...
        // A full width integer can never overflow, and `1 << 128` does not exist.
        shift < EmulatedIntegerPlaintextStorage::BITS && storage >= (1 << shift)
    }
}
