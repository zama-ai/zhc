...
use egui_graphs::{
    DisplayEdge, DisplayNode, Graph, GraphView, SettingsInteraction, SettingsNavigation,
    SettingsStyle,
};
use petgraph::{
    Directed, EdgeType,
...
impl<N, E> From<&StableGraph<N, E>> for Viewer<N, E>
where
    N: GraphShow + GraphFmt,
    E: GraphFmt,
{
    fn from(stable_graph: &StableGraph<N, E>) -> Self {
...
pub fn display<N, E>(graph: &StableGraph<N, E>, app_name: &str)
where
    N: GraphFmt + GraphShow,
    E: GraphFmt,
{
    run_native(
        ...
        Box::new(|_cc| Ok(Box::new(Viewer::from(graph)))),
    )
    .unwrap();
}
