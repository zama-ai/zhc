...
use crate::gir::{Dialect, OpId, IR};

use super::op_map::OpMap;
...
pub trait ForwardListScheduler {

    /// The IR dialect this scheduler operates on.
    type Dialect: Dialect;

    /// Extra configuration arguments for initialization.
    type Config;

    /// Creates a new scheduler instance for the given IR.
    ///
    /// This method is called once at the beginning of the scheduling process
    /// to initialize any scheduler-specific state based on the `ir` being scheduled.
    fn init(ir: &IR<Self::Dialect>, config: Self::Config) -> Self;
    ...
    /// for execution.
    fn select(&mut self, ready: impl Iterator<Item=Ready>) -> impl Iterator<Item=Selected> + '_;
...
impl<'i, D: Dialect> Tracker<'i, D> {
    ...
    fn from_ir(ir: &'i IR<D>) -> Self {
        let mut states = ir.new_filled_op_map(State::Retired);
        ir.ops_iter().for_each(|op| {
            match op.get_predecessors_iter().count() {
                0 => {states.insert(op.get_id(), State::Ready);}
                a => {states.insert(op.get_id(), State::Locked(a));}
            }
        });
        Self {
            states,
    ...
    fn ready_iter(&self) -> impl Iterator<Item = Ready> {
        self.states.iter().filter(|(_, s)| **s == State::Ready).map(|(opid, _)| Ready(opid))
    }
...
pub fn forward_list_scheduling<S: ForwardListScheduler>(ir: &IR<S::Dialect>, config: S::Config) -> Schedule {
    let mut sched = Schedule::empty();
    let mut tracker = Tracker::from_ir(ir);
    let mut selected = Vec::new();
    let mut simulator = S::init(ir, config);

    while !tracker.over() {
        let selection = simulator.select(tracker.ready_iter());
        selected.extend(selection);
        tracker.issue_selected(selected.iter().copied());
        sched.issue_selected(selected.iter().copied());
        let retired = simulator.advance();
        tracker.retire(retired);
        selected.clear();
    }
