...
impl<E: Event> Tracer<E> {
    pub fn new() -> Self {
        ...
        trace.display_time_unit = Some(crate::utils::tracing::Unit::Nanoseconds);
    ...
    pub fn add_counter<S: AsRef<str>>(&mut self, name: S, value: f64) {
        if !self.counter_trackers.contains_key(name.as_ref()) {
            ...
            self
                .trace
                .set_thread_name(COUNTERS_PID, tid, name.as_ref());
        }
    ...
    pub fn add_simulatable<S: Simulatable<Event = E>>(&mut self, simulatable: &S) {
        ...
        if !self.simulatable_trackers.contains_key(&address) {
            ...
            self
                .trace
                .set_thread_name(SIMULATABLES_PID, tid, &name);
