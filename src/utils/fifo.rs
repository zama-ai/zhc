use std::{mem::MaybeUninit, ops::{Index, IndexMut}, fmt::Debug};
use serde::Serialize;
...
impl<T> Fifo<T> {
    ...
    pub fn push_back(&mut self, val: T) {
        assert_ne!(self.len(), self.capacity(), "Tried to insert in a full fifo");
...
impl<T> Index<usize> for Fifo<T> {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        if index >= self.len {
            panic!("Index {} out of bounds for fifo of length {}", index, self.len);
        }
