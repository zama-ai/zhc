...
/// Number of heap slots a compiled stream needs: the largest slot its `LD`/`ST`
/// address, plus one. Zero if it never spills.
///
/// A host compares it against the heap band its board reserves before loading.
