...
use hpuc_ir::{Dialect, OpId, ValId, IR};
use petgraph::prelude::StableGraph;


pub trait IRExt {
    fn to_contextual_graph(
        self: Arc<Self>,
    ) -> StableGraph<(OpId, Arc<Self>), (ValId, Arc<Self>)>;
    ...
    fn to_contextual_graph(
        ...
        for value in self.walk_vals_linear() {
            for user in value.get_users_iter() {
                output.add_edge(map[value.get_origin().get_id()], map[user.get_id()], (value.get_id(), self.clone()));
